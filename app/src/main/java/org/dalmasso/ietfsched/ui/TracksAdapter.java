...
public class TracksAdapter extends CursorAdapter {
    ...
    @Override
    public int getCount() {
    	int sCount = super.getCount();
    	if (sCount > 0) {
   			sCount += mPositionDisplacement;
    	}
//    	Log.d(TAG, "getCount = " + sCount);
    	return sCount;
    }
...
//    	Log.d(TAG, "getItem pos " + position);
    ...
    public int getItemViewType(int position) {
    	int count = -1;
        if (mHasAllItem && position == 0) {
            count =  getViewTypeCount() - 1;
        } else {
        	count =  super.getItemViewType(position - mPositionDisplacement);
        }
//        Log.d(TAG, "getItemViewType pos / itemviewType " + position + " / " + count);
        return count;
    }
